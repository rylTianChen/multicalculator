#ifndef MATHCALC_CALC_H
#define MATHCALC_CALC_H

#include<string>
#include<QString>
typedef std::string str;
typedef long long ll;
#include"expr_func.h"
#include"tools/init_func.h"
#include"tools/hpcalc.h"
#include"log.h"

constexpr int NT_LEN_LIMIT = 15;

double StdCalcFunc(str ori_input_str){
    addLogLine(DEBUG, "Got into StdCalcFunc()");
    addIndent();

    init_op_lv();
    //head、tail均不存值
    Token<double>* head = new Token<double>;
    Token<double>* tail = new Token<double>;
    head->type = HEADTYPE;
    tail->type = TAILTYPE;
    head->prev = head, head->next = tail;
    tail->prev = head, tail->next = tail;

    addLogLine(INFO, "Expression: "+QString::fromStdString(ori_input_str));
    try{
        double_expr_read(head, ori_input_str);
    }catch(Err err_info){
        clear_tk(head);
        popIndent();
        addLogLine(DEBUG, "Exiting from StdCalcFunc() abnormally");
        throw err_info;
    }
    addLogLine(DEBUG, "Token list: "+QString::fromStdString(tklistTOstr(head)));

    double res;
    try{
        res = calculate<double>(head);
    }catch(Err err_info){
        clear_tk(head);
        popIndent();
        addLogLine(DEBUG, "Exiting from StdCalcFunc() abnormally");
        throw err_info;
    }
    addLogLine(INFO, "Result: "+QString::fromStdString(doubleTOstr(res)));
    clear_tk(head);

    popIndent();
    addLogLine(DEBUG, "Exiting from StdCalcFunc()");
    return res;
}

HP HpCalcFunc(str ori_input_str){
    addLogLine(DEBUG, "Got into HpCalcFunc()");
    addIndent();

    init_op_lv();
    //head、tail均不存值
    Token<HP>* head = new Token<HP>;
    Token<HP>* tail = new Token<HP>;
    head->type = HEADTYPE;
    tail->type = TAILTYPE;
    head->prev = head, head->next = tail;
    tail->prev = head, tail->next = tail;

    addLogLine(INFO, "Expression: "+QString::fromStdString(ori_input_str));
    try{
        hp_expr_read(head, ori_input_str);
    }catch(Err err_info){
        clear_tk(head);
        popIndent();
        addLogLine(DEBUG, "Exiting from HpCalcFunc() abnormally");
        throw err_info;
    }
    addLogLine(DEBUG, "Token list: "+QString::fromStdString(tklistTOstr(head)));

    HP res;
    try{
        res = calculate<HP>(head);
    }catch(Err err_info){
        clear_tk(head);
        popIndent();
        addLogLine(DEBUG, "Exiting from HpCalcFunc() abnormally");
        throw err_info;
    }
    addLogLine(INFO, "Result: "+QString::fromStdString(str(res)));
    clear_tk(head);

    popIndent();
    addLogLine(DEBUG, "Exiting from HpCalcFunc()");
    return res;
}

ll fast_pow(ll a, int b){
    ll ans = 1;
    while(b > 0){
        if(b & 1) ans *= a, b--;
        else a = a*a, b >>= 1;
    }
    return ans;
}
str NtSqrtFunc(str ori_input_str){
    addLogLine(DEBUG, "Got into NtSqrtFunc()");
    addIndent();

    addLogLine(INFO, "Number: "+QString::fromStdString(ori_input_str));
    HP ori_num(ori_input_str);
    if(ori_num.isEMPTY() || ori_num.sign()<=0){
        popIndent();
        addLogLine(DEBUG, "Exiting from NtSqrtFunc() abnormally");
        throw Err(NOT_POSI_INT, -1, -1);
    }
    if(ori_num.size() > NT_LEN_LIMIT){
        popIndent();
        addLogLine(DEBUG, "Exiting from NtSqrtFunc() abnormally");
        throw Err(LARGE_NUM, -1, -1);
    }
    if(ori_num == 1) return "1";

    ll coef = 1, sqrt_num = 1;
    ll i, n = ori_num;
    int exp;
    for(i=2; i*i<=n; ){
        exp = 0;
        while(n%i == 0){
            n /= i;
            exp++;
        }
        if(exp > 0){
            coef *= fast_pow(i, exp>>1); // exp>>1 == exp/2
            if(exp & 1){ // exp&1 == exp%2==1
                sqrt_num *= i;
            }
        }

        if(i == 2) i++;
        else i += 2; // 优化循环次数
    }
    sqrt_num *= n;

    str res = "";
    if(coef > 1) res += str(HP(coef));
    if(sqrt_num > 1){
        res += "√" + str(HP(sqrt_num));
    }
    addLogLine(INFO, "Result: "+QString::fromStdString(res));

    popIndent();
    addLogLine(DEBUG, "Exiting from NtSqrtFunc()");
    return res;
}
str NtFactorFunc(str ori_input_str){
    addLogLine(DEBUG, "Got into NtFactorFunc()");
    addIndent();

    addLogLine(INFO, "Number: "+QString::fromStdString(ori_input_str));
    HP ori_num(ori_input_str);
    if(ori_num.isEMPTY() || ori_num.sign()<=0){
        popIndent();
        addLogLine(DEBUG, "Exiting from NtFactorFunc() abnormally");
        throw Err(NOT_POSI_INT, -1, -1);
    }
    if(ori_num.size() > NT_LEN_LIMIT){
        popIndent();
        addLogLine(DEBUG, "Exiting from NtFactorFunc() abnormally");
        throw Err(LARGE_NUM, -1, -1);
    }
    if(ori_num == 1) return "1";

    char is_first_factor = true;
    ll i, n = ori_num;
    int exp = 0;
    str res = "";
    for(i=2; i*i<=n; ){
        exp = 0;
        while(n%i == 0){
            n /= i;
            exp++;
        }
        if(exp > 0){
            if(is_first_factor) is_first_factor = false;
            else res += " * ";
            res += str(HP(i));
            if(exp > 1) res += "^"+str(HP(exp));
        }

        if(i == 2) i++;
        else i += 2; // 优化循环次数
    }
    if (n > 1) {
        if (!is_first_factor) res += " * ";
        res += str(HP(n));
    }
    addLogLine(INFO, "Result: "+QString::fromStdString(res));

    popIndent();
    addLogLine(DEBUG, "Exiting from NtFactorFunc()");
    return res;
}
ll gcd(ll a, ll b){
    ll t;
    while(b != 0){
        t = a;
        a = b, b = t%b;
    }
    return a;
    // A recursion version of gcd()
    // if(b == 0) return a;
    // else return hp_gcd(b, a%b);
}
str NtGcdFunc(str ori_str1, str ori_str2){
    addLogLine(DEBUG, "Got into NtGcdFunc()");
    addIndent();

    addLogLine(INFO, "Numbers: "
                          +QString::fromStdString(ori_str1) + " "
                          +QString::fromStdString(ori_str2));
    HP a(ori_str1), b(ori_str2);
    if(a.isEMPTY() || a.sign()<=0){
        popIndent();
        addLogLine(DEBUG, "Exiting from NtGcdFunc() abnormally");
        throw Err(NOT_POSI_INT, -1, -1);
    }
    if(b.isEMPTY() || b.sign()<=0){
        popIndent();
        addLogLine(DEBUG, "Exiting from NtGcdFunc() abnormally");
        throw Err(NOT_POSI_INT, -1, -1);
    }
    if(a.size() > NT_LEN_LIMIT){
        popIndent();
        addLogLine(DEBUG, "Exiting from NtGcdFunc() abnormally");
        throw Err(LARGE_NUM, -1, -1);
    }
    if(b.size() > NT_LEN_LIMIT){
        popIndent();
        addLogLine(DEBUG, "Exiting from NtGcdFunc() abnormally");
        throw Err(LARGE_NUM, -1, -1);
    }

    if(a==1 || b==1) return "1";
    ll gcd_num = gcd(ll(a), ll(b));
    addLogLine(INFO, "Result: "+QString::fromStdString(str(HP(gcd_num))));

    popIndent();
    addLogLine(DEBUG, "Exiting from NtGcdFunc()");
    return str(HP(gcd_num));
}
HP lcm(HP a, HP b){
    return a*b/gcd(ll(a), ll(b));
}
str NtLcmFunc(str ori_str1, str ori_str2){
    addLogLine(DEBUG, "Got into NtLcmFunc()");
    addIndent();

    addLogLine(INFO, "Numbers: "
                          +QString::fromStdString(ori_str1) + " "
                          + QString::fromStdString(ori_str2));
    HP a(ori_str1), b(ori_str2);
    if(a.isEMPTY() || a.sign()<=0){
        popIndent();
        addLogLine(DEBUG, "Exiting from NtLcmFunc() abnormally");
        throw Err(NOT_POSI_INT, -1, -1);
    }
    if(b.isEMPTY() || b.sign()<=0){
        popIndent();
        addLogLine(DEBUG, "Exiting from NtLcmFunc() abnormally");
        throw Err(NOT_POSI_INT, -1, -1);
    }
    if(a.size() > NT_LEN_LIMIT){
        popIndent();
        addLogLine(DEBUG, "Exiting from NtLcmFunc() abnormally");
        throw Err(LARGE_NUM, -1, -1);
    }
    if(b.size() > NT_LEN_LIMIT){
        popIndent();
        addLogLine(DEBUG, "Exiting from NtLcmFunc() abnormally");
        throw Err(LARGE_NUM, -1, -1);
    }

    if(a==1 || b==1) return "1";
    HP lcm_num = lcm(a, b);
    addLogLine(INFO, "Result: "+QString::fromStdString(str(lcm_num)));

    popIndent();
    addLogLine(DEBUG, "Exiting from NtLcmFunc()");
    return str(lcm_num);
}

#endif
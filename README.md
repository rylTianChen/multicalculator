# High-Precision Multi-Function Calculator

[![License](https://img.shields.io/badge/license-LGPL--3.0-blue.svg)]()
[![Qt](https://img.shields.io/badge/Qt-6.11.1-brightgreen.svg)](https://qt.io)
[![Platform](https://img.shields.io/badge/platform-Windows-lightgrey.svg)]()
![C++17](https://img.shields.io/badge/Language-C%2B%2B11-blue)
![Platform](https://img.shields.io/badge/Platform-Windows10%7CWindows11-lightgray)

A powerful multi-mode calculator with high-precision integer arithmetic, standard decimal calculations, and number theory tools.

---

## ✨ Features

### Standard Mode
- Decimal arithmetic with `+ - * / ^` and parentheses
- Daily use with good precision

### High-Precision Mode
- Arbitrary-precision integer arithmetic (up to 1e8 digits)
- Supports `+ - * / ^ % & | !` (bitwise operations)
- Automatic scientific notation for results over 100 digits

### Number Theory Tools
- **Radical Simplification**: Simplify square roots (e.g., `√72 → 6√2`)
- **Prime Factorization**: Break down integers into prime factors
- **GCD / LCM**: Compute greatest common divisor and least common multiple

---

## 🖥️ Screenshots

### Standard Mode
Basic decimal arithmetic for everyday calculations.

![Standard Mode](screenshots/std_mode_shot.jpg)

### High-Precision Mode
Arbitrary-precision integer arithmetic with bitwise operations and automatic scientific notation for large results.

![High-Precision Mode](screenshots/hp_mode_shot.jpg)

### Number Theory Tools
Simplify radicals, factor integers, and compute GCD/LCM.

![Number Theory Mode](screenshots/nt_mode_shot.jpg)

---

## 📦 System Requirements

- **Windows 10 / 11**
- No additional installation required (standalone executable)

---

## 🚀 Quick Start

1. Download the latest release from [Releases](../../releases)
2. Extract the zip file
3. Double-click `multicalculator.exe` to run
